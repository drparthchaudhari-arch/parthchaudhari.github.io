;(function () {
  ...
  function getLocalStats() {
    ...
    if (storage) {
      return {
        casesCompleted:
          typeof storage.countCaseCompletions === 'function'
            ? storage.countCaseCompletions()
            : 0,
        currentStreak:
          typeof storage.getCurrentStreak === 'function'
            ? storage.getCurrentStreak()
            : 0,
        lastSyncedAt:
          typeof storage.getLastSyncedAt === 'function'
  ...
  function exportData() {
    ...
    try {
      ...
      var fileName =
        'parthchaudhari-data-' + new Date().toISOString().slice(0, 10) + '.json'
      toDownloadFile(
      ...
      reader.onload = function (event) {
        try {
          resolve(
            JSON.parse(
              String(
                event.target && event.target.result ? event.target.result : '{}'
              )
            )
          )
        } catch (error) {
  ...
  function renderSyncState(snapshot) {
    ...
    var user =
      snapshot && snapshot.user
        ? snapshot.user
        : getSync() && typeof getSync().getCurrentUser === 'function'
          ? getSync().getCurrentUser()
          : null
    var stats = getLocalStats()
    ...
    if (lastSynced) {
      var syncedAt =
        snapshot && snapshot.lastSyncedAt
  ...
  async function importDataFromFile(event) {
    ...
    try {
      ...
      if (result && result.success) {
        setMessage(
          'Import complete. ' + result.imported + ' keys updated.',
          'success'
        )
  ...
  async function sendMagicLink() {
    ...
    setMessage(
      'Failed to send magic link. Please verify email auth settings in Supabase.',
      'error'
    )
  }
  ...
  async function syncNow() {
    ...
    try {
      ...
      if (result && result.ok) {
        ...
        setMessage(
          'Sync skipped or failed. Local mode is still active.',
          'error'
        )
      }
    } catch (error) {
