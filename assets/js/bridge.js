;(function () {
  ...
  function triggerBackgroundSync(trigger) {
    ...
    window.pcSync
      .syncToServer({ trigger: trigger || 'bridge_update' })
      .catch(function () {
  ...
  function initDiagnoseBlock(block) {
    ...
    var correctLabel =
      form.getAttribute('data-correct-label') || 'Correct option'
    ...
    var studyLabel =
      form.getAttribute('data-study-label') || 'Open WordWeb NAVLE Topic List'
    ...
    button.addEventListener('click', function () {
      ...
      if (!selected) {
        feedback.innerHTML =
          '<p><strong>Select one option.</strong> Choose A, B, or C and check again.</p>'
        return
      }

      var selectedText = selected.parentElement
        ? selected.parentElement.textContent.trim()
        : selected.value
      var intro =
        selected.value === correctOption
      ...
      feedback.innerHTML =
        ...
        explanation +
        '</p>' +
        '<p><strong>NAVLE Teaching Point:</strong> ' +
        teaching +
        '</p>' +
        '<div class="pc-panel-actions"><a class="pc-btn pc-btn--secondary" href="' +
        studyLink +
        '">' +
        studyLabel +
        '</a></div>'
      ...
      if (caseKey) {
        if (
          window.pcStorage &&
          typeof window.pcStorage.markCaseCompleted === 'function'
        ) {
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initBridgeInteractions, {
      once: true,
    })
  } else {
