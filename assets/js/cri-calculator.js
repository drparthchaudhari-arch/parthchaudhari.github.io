;(function () {
  ...
  function renderResult(result) {
    ...
    setText(
      'cri-final-conc',
      format(result.finalConcentration, 4) +
        ' ' +
    ...
    setText(
      'cri-verify',
      format(result.backCalculatedRate, 2) + ' ' + unitWord(result.targetUnit)
    )

    var stepText =
      '' +
      '1) Remove ' +
      format(result.fluidToRemoveMl, 2) +
      ' mL from a ' +
      format(result.bagVolumeMl, 0) +
      ' mL bag. ' +
      '2) Add ' +
      format(result.volumeToAddMl, 2) +
      ' mL of ' +
      result.drugName +
      '. ' +
      '3) Mix and label concentration as ' +
      format(result.finalConcentration, 4) +
      ' ' +
      result.concentrationUnit +
      '/mL. ' +
      '4) Run at ' +
      format(result.adminRateMlPerHr, 1) +
      ' mL/hr (' +
      format(result.dripRateDropsPerMin, 0) +
      ' drops/min at 20 gtt/mL).'
    ...
    setText(
      ...
      'Reference: ' +
        result.reference +
        '. Verification delta: ' +
        format(result.verificationPercent, 2) +
        '%.'
    )
  }
  ...
  function calculate(event) {
    ...
    var stockConcentration = toNumber(
      document.getElementById('cri-concentration').value
    )
    ...
    if (
      ...
      setSafety([
        {
          level: 'danger',
          text: 'Invalid input detected. Re-check weight, rate, concentration, bag volume, and duration.',
        },
      ])
      return
    }
    ...
    if (!Number.isFinite(basePerKgMin) || basePerKgMin <= 0) {
      setText(
        'cri-steps',
        'Selected rate unit is not supported for this calculation.'
      )
      setText('cri-meta', '-')
      setSafety([
        {
          level: 'danger',
          text: 'Unsupported unit conversion. Choose a valid CRI rate unit.',
        },
      ])
      return
    }
    ...
    if (
      ...
      (basePerKgMin < rangeMin || basePerKgMin > rangeMax)
    ) {
      messages.push({
        level: 'caution',
        text:
          'Ordered rate is outside the typical range for ' + drug.name + '.',
      })
    }
    ...
    if (finalConcentration > drug.maxSafeConcentration) {
      messages.push({
        level: 'danger',
        text: 'Final bag concentration exceeds suggested safety threshold. Re-check protocol and dilution.',
      })
    }
    ...
    if (volumeToAdd > bagVolume * 0.35) {
      messages.push({
        level: 'danger',
        text: 'Large additive volume relative to bag size. Consider concentrated preparation or a larger bag.',
      })
    }
    ...
    if (verificationPercent > 1) {
      messages.push({
        level: 'caution',
        text: 'Back-calculated rate differs from target by more than 1%.',
      })
    }
    ...
    if (drug.lightSensitive) {
      messages.push({
        level: 'caution',
        text: 'This CRI is light sensitive. Use light-protective handling per hospital policy.',
      })
    }
    ...
    if (drug.requiresFilter) {
      messages.push({
        level: 'caution',
        text: 'Inline filter requirement may apply for this infusion.',
      })
    }
    ...
    if (drug.incompatibleWith.length) {
      messages.push({
        level: 'caution',
        text:
          'Known compatibility caution: ' +
          drug.incompatibleWith.join(', ') +
          '.',
      })
    }
