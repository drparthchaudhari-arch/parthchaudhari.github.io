;(function () {
  ...
  function renderConcentrationOptions(drug) {
    ...
    if (
      !drug ||
      !Array.isArray(drug.concentrations) ||
      !drug.concentrations.length
    ) {
      if (hint) {
        hint.textContent =
          'No preset concentrations for this drug. Enter concentration manually.'
      }
      return
    }
    ...
    if (hint) {
      var categoryLabel = drug.category
  ...
  function updateConcentrations() {
    ...
    if (referenceHint) {
      if (drug && drug.reference) {
        referenceHint.textContent =
          'Reference baseline: ' + drug.reference + '.'
      } else if (drug) {
        referenceHint.textContent =
          'Reference baseline: DailyMed + Merck Vet Manual + approved site reference map.'
      } else {
  ...
  function calculateDose(event) {
    ...
    var concentration = parseFloat(
      byId('concentration') ? byId('concentration').value : ''
    )
    ...
    if (firstInvalid || !species || !weight || !drugName || !concentration) {
      setFormMessage(
        'Please complete species, weight, drug, and concentration before calculating.',
        true
      )
    ...
    if (mgPerDose > maxDose && maxDose > 0) {
      setResults(
        ...
        mlPerDose.toFixed(2) + ' mL',
        totalDaily.toFixed(2) + ' mg/day',
        'Warning: Dose exceeds typical maximum (' +
          maxDose.toFixed(2) +
          ' mg). Verify with veterinarian.'
      )
    }
  }
