;(function () {
  ...
  function applyPrefillFromQuery() {
    ...
    prefilled =
      setInputValue('fluid-weight', params.get('weight'), false) || prefilled
    prefilled =
      setInputValue('fluid-dehydration', params.get('dehydration'), true) ||
      prefilled
    prefilled =
      setInputValue('fluid-maintenance', params.get('maintenance'), false) ||
      prefilled
  ...
  function readInputs() {
    ...
    var dehydrationPercent = toNumber(
      document.getElementById('fluid-dehydration').value
    )
    var maintenanceFactor = toNumber(
      document.getElementById('fluid-maintenance').value
    )
    var userOverride = !!(
      document.getElementById('fluid-override') &&
      document.getElementById('fluid-override').checked
    )
  ...
  function renderCalculation(input) {
    ...
    if (shockNode) {
      ...
      shockNode.textContent =
        'Shock bolus reminder: dog ' +
        formatNumber(dogLow, 0) +
        '-' +
        formatNumber(dogHigh, 0) +
        ' mL aliquots; ' +
        'cat ' +
        formatNumber(catLow, 0) +
        '-' +
        formatNumber(catHigh, 0) +
        ' mL aliquots, then reassess perfusion.'
    }
