;(function () {
  ...
  function applyPrefillFromQuery() {
    ...
    prefilled =
      setInputValue('icp-weight', params.get('weight'), false) || prefilled
    prefilled =
      setInputValue('icp-dose', params.get('dose'), false) || prefilled
    prefilled =
      setInputValue('icp-bag-volume', params.get('bag'), false) || prefilled
    prefilled =
      setInputValue('icp-insulin-units', params.get('units'), false) ||
      prefilled
  ...
  function readInputs() {
    ...
    var insulinUnits = toNumber(
      document.getElementById('icp-insulin-units').value
    )
    var bg = toNumber(document.getElementById('icp-bg').value)
    var userOverride = !!(
      document.getElementById('icp-override') &&
      document.getElementById('icp-override').checked
    )
  ...
  function renderCalculation(input) {
    ...
    var referenceLabel =
      format(referenceUnits, 2) +
      ' U expected for ' +
      format(bagVolume, 0) +
      ' mL bag' +
      ' (' +
      (referenceDelta >= 0 ? '+' : '') +
      format(referenceDelta, 2) +
      ' U vs entered)'
    ...
    setText(
      'icp-dextrose',
      format(dextroseLow, 1, '') + ' - ' + format(dextroseHigh, 1, ' g/hr')
    )
    ...
    if (Number.isFinite(bg)) {
      if (bg < 150) {
        notes.push(
          'BG is already low; hold or reduce insulin and increase dextrose support per clinician protocol.'
        )
      } else if (bg < 250) {
        notes.push(
          'BG is near transition range; continue insulin but add dextrose support and monitor closely.'
        )
      } else {
        notes.push(
          'BG remains elevated; continue scheduled reassessment while titrating CRI.'
        )
      }
    }
    ...
    if (requiredUnitsPerHour > 2) {
      notes.push(
        'High hourly insulin exposure: verify dilution math and pump programming with a second check.'
      )
    }

    notes.push(
      'Recheck BG/electrolytes every 2-4 hours and adjust CRI/dextrose together.'
    )
  ...
  function calculate(event) {
    ...
    if (!input) {
      setText(
        'icp-note',
        'Enter valid positive values for weight, dose, bag volume, and insulin units.'
      )
      return
    }
