(function () {
    ...
    function setConsentCookie(value) {
        ...
        document.cookie =
            ...
            '; path=/; max-age=' + CONSENT_COOKIE_MAX_AGE +
    ...
    function createConsentEvent(consent) {
        if (typeof window.CustomEvent === 'function') {
            return new CustomEvent('pc-consent-updated', { detail: { consent: consent || null } });
        }
    ...
    function loadDeferredScriptsForCategory(category) {
        ...
        var selector = 'script[type="text/plain"][data-pc-consent-category="' + category + '"]';
    ...
    function ensureCookieBanner(consent) {
        ...
        if (saveBtn) {
            saveBtn.addEventListener('click', function () {
                ...
                var functionalAllowed = functionalInput ? !!functionalInput.checked : false;
    ...
    function addToolEducationalWarning() {
        ...
        wrapper.innerHTML =
            ...
            '<input type="checkbox" id="pc-tool-ack-checkbox" ' + (previouslyAcknowledged ? 'checked' : '') + '>' +
        ...
        document.addEventListener('submit', function (event) {
            ...
            if (message) {
                message.textContent = 'Please confirm educational-use acknowledgement before generating results.';
            }
            checkbox.focus();
        }, true);
    }
    ...
    function appendTrademarkFooterIfMissing() {
        if (document.querySelector('.pc-footer') || document.querySelector('[data-pc-legal-mini-footer]')) {
            return;
        }
    ...
    function addGameAttributionIfNeeded() {
        ...
        if (path.indexOf('/play/2048/') === 0 || path === '/games/2048.html' || path.indexOf('/games/2048/') === 0) {
    ...
    function appendGameframeDisclaimer() {
        if (!document.body || !document.body.classList.contains('pc-page--gameframe')) {
            return;
        }
    ...
    function canUseAnalytics() {
        return !!(state.consent && state.consent.analytics === true && isConsentFresh(state.consent));
    }

    function canUseFunctional() {
        return !!(state.consent && state.consent.functional === true && isConsentFresh(state.consent));
    }
