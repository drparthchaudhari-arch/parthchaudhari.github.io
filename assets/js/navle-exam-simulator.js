(function () {
    ...
    function normalizeTopic(topic) {
        var value = String(topic || 'general').trim().toLowerCase();
        if (!value) {
            return 'general';
        }
        return value;
    ...
    function parseQuestions(payload) {
        ...
        return payload.filter(function (question) {
            if (!question || typeof question !== 'object') {
                return false;
            }

            if (!question.id || !question.stem || !question.options || !question.correct) {
                return false;
            }
    ...
    function rememberLearningLocation() {
        safeSet('pc_last_learning_url_v1', window.location.pathname + window.location.search);
    ...
    function hydrateSavedSession() {
        ...
        state.answers = Array.isArray(payload.answers) ? payload.answers.slice(0, hydratedQuestions.length) : [];
        ...
        state.marked = payload.marked && typeof payload.marked === 'object' ? payload.marked : {};
        state.currentIndex = clamp(Number(payload.currentIndex) || 0, 0, hydratedQuestions.length - 1);
        state.remainingSeconds = clamp(Number(payload.remainingSeconds) || EXAM_DURATION_SECONDS, 0, EXAM_DURATION_SECONDS);
        state.startedAtIso = typeof payload.startedAtIso === 'string' ? payload.startedAtIso : new Date().toISOString();
    ...
    function updateTimerDisplay() {
        ...
        if (timer) {
            ...
            timer.classList.toggle('pc-exam-timer--warning', state.remainingSeconds <= 300 && state.remainingSeconds > 60);
            timer.classList.toggle('pc-exam-timer--danger', state.remainingSeconds <= 60);
        }
    ...
    function renderQuestionGrid() {
        ...
        var html = state.questions.map(function (question, index) {
            ...
            return '' +
                '<button type="button" class="' + classes.join(' ') + '" data-nav-index="' + index + '"' +
                ' aria-label="Question ' + (index + 1) + ', ' + getQuestionStatusText(status) + '">' +
                '<span>' + (index + 1) + '</span>' +
                (marked ? '<span class="pc-exam-grid-flag" aria-hidden="true">M</span>' : '') +
                '</button>';
        }).join('');
    ...
    function renderQuestionCard() {
        ...
        var optionsHtml = sortedOptionEntries(question).map(function (entry) {
            var key = entry[0];
            ...
            return '' +
                '<button type="button" class="' + classes.join(' ') + '" data-option="' + escapeHtml(key) + '">' +
                '<span class="pc-option-key">' + escapeHtml(key) + '</span>' +
                '<span class="pc-option-text">' + escapeHtml(optionText) + '</span>' +
                '</button>';
        }).join('');
        ...
        container.innerHTML = '' +
            ...
            '<span class="pc-question-number">Question ' + (state.currentIndex + 1) + ' of ' + state.questions.length + '</span>' +
            '<span class="pc-question-topic">System: ' + escapeHtml(topicLabel(question.topic)) + '</span>' +
            markedChip +
            '</div>' +
            '<p class="pc-question-stem">' + escapeHtml(question.stem) + '</p>' +
    ...
    function renderQuestionProgress() {
        ...
        if (progress) {
            progress.textContent = 'Question ' + (state.currentIndex + 1) + ' of ' + questionTotal;
        }
        ...
        if (markBtn) {
            markBtn.textContent = isMarked(state.currentIndex) ? 'Unmark Question' : 'Mark Question';
        }
    ...
    function buildResult() {
        ...
        result.systemStats = Object.keys(systems).map(function (key) {
            return systems[key];
        }).sort(function (a, b) {
            if (b.total !== a.total) {
        ...
        result.scorePercent = result.total ? ((result.correct / result.total) * 100) : 0;
        return result;
    }
    ...
    function renderResult(result, reason) {
        ...
        if (summary) {
            summary.textContent = reason === 'time'
        ...
        if (systems) {
            systems.innerHTML = result.systemStats.map(function (stat) {
                var reviewText = stat.wrongQuestionNumbers.length
                    ? stat.wrongQuestionNumbers.join(', ')
                    : 'None';
                return '' +
                    '<article class="pc-exam-system-card">' +
                    '<h4>' + escapeHtml(topicLabel(stat.topic)) + '</h4>' +
                    '<p><strong>Correct:</strong> ' + stat.correct + ' / ' + stat.total + '</p>' +
                    '<p><strong>Wrong:</strong> ' + stat.wrong + '</p>' +
                    '<p><strong>Not Done:</strong> ' + stat.unanswered + '</p>' +
                    '<p class="pc-text-small"><strong>Review:</strong> ' + escapeHtml(reviewText) + '</p>' +
                    '</article>';
            }).join('');
        }
        ...
        if (review) {
            if (!result.reviewItems.length) {
                review.innerHTML = '<p class="pc-text-small">All questions were correct. Strong pass for this 24-question block.</p>';
            } else {
                review.innerHTML = result.reviewItems.map(function (item) {
                    var explanationHtml = item.explanation
                        ? '<p class="pc-text-small"><strong>Why:</strong> ' + escapeHtml(item.explanation) + '</p>'
                        : '';

                    return '' +
                        '<article class="pc-exam-review-item">' +
                        '<h4>Q' + item.number + ' - ' + escapeHtml(item.topic) + '</h4>' +
                        '<p><strong>Your answer:</strong> ' + escapeHtml(item.yourAnswer) + '</p>' +
                        '<p><strong>Correct answer:</strong> ' + escapeHtml(item.correctAnswer) + '</p>' +
                        '<p class="pc-text-small">' + escapeHtml(item.stem) + '</p>' +
                        explanationHtml +
                        '</article>';
                }).join('');
            }
    ...
    function attemptSubmit() {
        ...
        if (notDone > 0) {
            var shouldSubmit = window.confirm('You still have ' + notDone + ' not done question(s). Submit anyway?');
            if (!shouldSubmit) {
                return;
            }
        }
    ...
    function openComingSoonModal(modeLabel) {
        ...
        if (copy) {
            copy.textContent = 'The ' + modeLabel + ' is coming soon. Right now, use the live 24-question mode while we build all six 60-question NAVLE blocks.';
        }
    ...
    function setStartButtonsDisabled(disabled) {
        [byId('hero-start-btn'), byId('start-exam-24-btn'), byId('coming-soon-start-live-btn')].forEach(function (button) {
            if (button) {
    ...
    function evaluateCalc() {
        ...
        try {
            ...
            var normalized = Math.abs(value) < 1e12 ? value : Number(value.toPrecision(12));
    ...
    async function loadQuestions() {
        ...
        if (!response.ok) {
            throw new Error('HTTP ' + response.status + ' while loading question bank.');
        }

        var payload = await response.json();
        var questions = parseQuestions(payload);
        if (questions.length < SIMULATOR_QUESTION_COUNT) {
            throw new Error('Question bank requires at least ' + SIMULATOR_QUESTION_COUNT + ' items.');
        }
    ...
    async function init() {
        ...
        try {
            ...
            setLaunchError('Could not load the 24-question simulator right now. Please refresh and try again.');
            return;
        }
        ...
        var resume = window.confirm('Resume your in-progress 24-question simulator attempt?');
        if (resume) {
            resumeExam();
            return;
        }
