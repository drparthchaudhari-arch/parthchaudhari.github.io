;(function () {
  ...
  function sortResults(left, right) {
    ...
    if (
      left.entry.date &&
      right.entry.date &&
      left.entry.date !== right.entry.date
    ) {
  ...
  function renderEmptyState() {
    ...
    for (var i = 0; i < POPULAR_LINKS.length; i += 1) {
      links +=
        '<a class="pc-link-chip" href="' +
        POPULAR_LINKS[i].url +
        '">' +
        escapeHtml(POPULAR_LINKS[i].label) +
        '</a>'
    }
    ...
    elements.results.innerHTML =
      ...
      '<div class="pc-search-popular-links">' +
      links +
      '</div>' +
      '</section>'
  ...
  function renderNoResults(query) {
    ...
    elements.results.innerHTML =
      '<section class="pc-search-empty">' +
      '<h3>No results</h3>' +
      '<p>No matches for <strong>' +
      escapeHtml(query) +
      '</strong>. Try broader terms like cardio, endocrine, or wordweb topics.</p>' +
      '</section>'
  ...
  function buildGroup(matches, type, terms, linkOffset) {
    ...
    for (var j = 0; j < items.length; j += 1) {
      ...
      html +=
        '<a href="' +
        item.url +
        '" data-pc-result-link data-result-index="' +
        resultIndex +
        '" tabindex="-1">' +
        createHighlight(item.title, terms) +
        '</a>'
  ...
  function renderResults(query) {
    ...
    state.resultLinks = elements.results.querySelectorAll(
      '[data-pc-result-link]'
    )
    state.activeResultIndex = -1
  ...
  function handleInputKeydown(event) {
    ...
    if (event.key === 'ArrowUp') {
      event.preventDefault()
      setActiveResult(
        state.activeResultIndex <= 0 ? 0 : state.activeResultIndex - 1
      )
      return
    }
