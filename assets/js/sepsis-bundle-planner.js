;(function () {
  ...
  function render(event) {
    ...
    setText(
      ...
      format(bolusLow, 0, '') +
        ' - ' +
        format(bolusHigh, 0, ' mL aliquot') +
        ' | Shock ceiling ' +
        format(shockLow, 0, '') +
        '-' +
        format(shockHigh, 0, ' mL total')
    )
    setText(
      'sbp-reassess',
      reassessWindow + ' min reassessment cycle (per bolus)'
    )
    ...
    if (Number.isFinite(map) && map < 60) {
      perfusionFlags.push(
        'MAP is low; vasopressor planning may be needed if fluid response is limited.'
      )
    } else if (Number.isFinite(map) && map < 65) {
      perfusionFlags.push(
        'MAP is borderline; monitor closely during each reassessment cycle.'
      )
    }
    if (!perfusionFlags.length) {
      perfusionFlags.push(
        'Use perfusion trend (mentation, pulse quality, lactate, urine output) to drive next step.'
      )
    }

    setText(
      'sbp-antimicrobials',
      'Collect culture/cytology samples if this does not delay treatment, then begin broad-spectrum antimicrobials within the first hour.'
    )
    setText(
      'sbp-source',
      'Define likely source early (abdomen, urinary, respiratory, wound, line-related), and plan source control (drainage/debridement/surgery) in parallel.'
    )
