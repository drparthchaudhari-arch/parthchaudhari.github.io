...
PCStorage.getCurrentStreak = function getCurrentStreak() {
  const streak = this.getStreak()
  return streak && Number.isFinite(Number(streak.current))
...
PCStorage.markCaseCompleted = function markCaseCompleted(caseKey) {
  ...
  const storageKey =
    normalized.indexOf('pc_case_') === 0 ? normalized : 'pc_case_' + normalized
...
PCStorage.buildLocalSyncPayload = function buildLocalSyncPayload() {
  ...
  return {
    ...
    game_activity: {
      data:
        gameActivity && typeof gameActivity === 'object' ? gameActivity : {},
...
PCStorage.applySyncData = function applySyncData(mergedPayload) {
  ...
  const caseField = pcUnwrapSyncField(
    mergedPayload && mergedPayload.case_completions
  )
  const caseData =
    caseField.data && typeof caseField.data === 'object' ? caseField.data : {}
  ...
  Object.keys(caseData).forEach((key) => {
    ...
    if (value === true || value === 'completed') {
      ...
      if (!pcSafeGetRaw(dateKey)) {
        pcSafeSetRaw(
          dateKey,
          normalizedCaseCompletion[caseId].completedAt ||
  ...
  const studyField = pcUnwrapSyncField(
    mergedPayload && mergedPayload.study_plan
  )
  const studyData =
    studyField.data && typeof studyField.data === 'object'
      ? studyField.data
      : {}
  ...
  const gameField = pcUnwrapSyncField(
    mergedPayload && mergedPayload.game_activity
  )
  const gameData =
    gameField.data && typeof gameField.data === 'object' ? gameField.data : {}
