;(function () {
  ...
  function dispatchStatusChange() {
    ...
    if (typeof window.CustomEvent === 'function') {
      window.dispatchEvent(
        new CustomEvent('pc-auth-status-change', { detail: snapshot })
      )
    }
  }
  ...
  function mergeData(localData, serverData) {
    ...
    var mergedCase = mergeField(
      safeLocal.case_completions,
      safeServer.case_completions
    )
    var mergedStudy = mergeField(safeLocal.study_plan, safeServer.study_plan)
    var mergedGame = mergeField(
      safeLocal.game_activity,
      safeServer.game_activity
    )
  ...
  function getLeaderboardStats(payload) {
    ...
    var currentStreak =
      studyData && Number.isFinite(Number(studyData.currentStreak))
  ...
  async function syncToServer(options) {
    ...
    try {
      ...
      if (!profileResult.ok) {
        if (
          profileResult.error &&
          isRlsViolation(profileResult.error, 'profiles')
        ) {
          return {
            ok: false,
            error: {
              ...
              message:
                'Sync blocked: profiles row could not be created due RLS policy. Run sync RLS hotfix migration and retry.',
            },
          }
        }
        ...
        return {
          ...
          error: profileResult.error || {
            ...
            message:
              'Sync blocked: unable to create profile row required before writing user_progress.',
          },
        }
      }
      ...
      var progressPayload = {
        ...
        case_completions: {
          data:
            localPayload.case_completions && localPayload.case_completions.data
              ? localPayload.case_completions.data
              : {},
          updatedAt: updatedAt,
        },
        study_plan: {
          data:
            localPayload.study_plan && localPayload.study_plan.data
              ? localPayload.study_plan.data
              : {},
          updatedAt: updatedAt,
        },
        game_activity: {
          data:
            localPayload.game_activity && localPayload.game_activity.data
              ? localPayload.game_activity.data
              : {},
          updatedAt: updatedAt,
        },
        updated_at: updatedAt,
      ...
      var progressResult = await client
        .from('user_progress')
      ...
      try {
        ...
        if (isRlsViolation(leaderboardError, 'leaderboard')) {
          warnings.push(
            'Leaderboard sync skipped due leaderboard table RLS policy.'
          )
        } else {
          warnings.push(
            'Leaderboard sync skipped: ' + getErrorMessage(leaderboardError)
          )
        }
      }
  ...
  function resolveAuthRedirect(options, fallbackPath) {
    var redirectTo =
      window.location.origin + String(fallbackPath || '/account/')
  ...
  async function signUpWithPassword(email, password, options) {
    ...
    try {
      ...
      var session =
        response.data && response.data.session ? response.data.session : null
    ...
    try {
      ...
      if (response.error) {
        ...
        if (
          ...
          isRedirectPolicyError(response.error)
        ) {
  ...
  function isMissingRpcError(error) {
    ...
    return (
      ...
      (message.indexOf('function public.') !== -1 &&
  ...
  function readConsentFromUserMetadata(user) {
    ...
    if (
      ...
      typeof metadata.marketing_consent === 'object'
    ) {
      payload.marketing = {
        text:
          metadata.marketing_consent.text ||
          'Yes, send me study tips and NAVLE updates.',
  ...
  async function exportAllData() {
    ...
    try {
      ...
      if (profileResult.error) {
        payload.warnings.push(
          'profiles export failed: ' + getErrorMessage(profileResult.error)
        )
      } else {
    ...
    try {
      ...
      if (progressResult.error && progressResult.error.code !== 'PGRST116') {
        payload.warnings.push(
          'user_progress export failed: ' +
            getErrorMessage(progressResult.error)
        )
      } else {
      ...
      payload.warnings.push(
        'user_progress export failed: ' + getErrorMessage(error)
      )
    }

    try {
      var leaderboardResult = await client
        .from('leaderboard')
        .select('*')
        .eq('user_id', user.id)
        .limit(1)
      ...
      if (
        ...
        leaderboardResult.error.code !== 'PGRST116'
      ) {
        payload.warnings.push(
          'leaderboard export failed: ' +
            getErrorMessage(leaderboardResult.error)
        )
      } else {
      ...
      payload.warnings.push(
        'leaderboard export failed: ' + getErrorMessage(error)
      )
    }
  ...
  async function deleteAccount() {
    ...
    if (!hardDeleted) {
      return {
        ok: false,
        error: new Error(
          'Hard delete requires delete_my_account RPC migration in Supabase'
        ),
        warnings: warnings,
      }
    }
  ...
  function handleAuthEvent(event, session) {
    if (session && session.user && isSessionExpired()) {
      ...
      if (
        timeoutClient &&
        timeoutClient.auth &&
        typeof timeoutClient.auth.signOut === 'function'
      ) {
