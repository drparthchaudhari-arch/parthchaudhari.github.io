;(function () {
  ...
  function calculate(event) {
    ...
    var component = String(
      document.getElementById('tx-component').value || 'prbc'
    )
    ...
    var plannedVolumeInput = toNumber(
      document.getElementById('tx-planned').value
    )
    ...
    var volumeNeeded =
      Number.isFinite(pcvDeficit) && pcvDeficit > 0
        ? (pcvDeficit * totalBloodVolume) / donor
        : NaN

    var plannedVolume =
      Number.isFinite(plannedVolumeInput) && plannedVolumeInput > 0
        ? plannedVolumeInput
        : volumeNeeded
    ...
    var maintenanceRate = Number.isFinite(plannedVolume)
      ? Math.min(plannedVolume / 4, maxRate)
      : NaN
    var maintenanceLabel = Number.isFinite(maintenanceRate)
      ? format(maintenanceRate, 1) +
        ' mL/hr (capped at ' +
        format(maxRate, 1) +
        ' mL/hr)'
      : '-'
    ...
    if (previousTransfusion && !crossmatchDone) {
      ...
      alertMessage =
        'Previous transfusion history detected without crossmatch. Stop and perform compatibility testing.'
      alertLevel = 'danger'
    } else if (previousTransfusion && crossmatchDone) {
      compatText = 'Crossmatch documented.'
      alertMessage =
        'Crossmatch confirmed. Continue with close reaction monitoring.'
      alertLevel = 'caution'
    }
    ...
    if (
      !alertMessage &&
      (!Number.isFinite(volumeNeeded) || volumeNeeded <= 0)
    ) {
      alertMessage =
        'Target PCV is not above current PCV. Volume-needed equation requires positive PCV deficit.'
      alertLevel = 'caution'
    }

    if (
      Number.isFinite(plannedVolume) &&
      componentData.maxMlKg &&
      plannedVolume > componentData.maxMlKg * weight
    ) {
      alertMessage =
        'Planned volume exceeds typical ' +
        componentData.label +
        ' range (' +
        componentData.doseRange +
        '). Re-check endpoint and component choice.'
      alertLevel = 'caution'
    }

    if (Number.isFinite(expectedRise) && expectedRise > 15) {
      alertMessage =
        'Large predicted PCV rise. Consider staged transfusion and recheck PCV before completing full volume.'
      alertLevel = 'caution'
    }
    ...
    var note =
      componentData.label +
      ': ' +
      componentData.guidance +
      ' Suggested start rate ' +
      componentData.startRate +
      '. '
    if (Number.isFinite(plannedVolume) && plannedVolume > 0) {
      note +=
        'Planned volume ' +
        format(plannedVolume, 1) +
        ' mL with expected PCV rise about ' +
        format(expectedRise, 1) +
        '%. '
    }
    note +=
      'Recheck perfusion and post-transfusion PCV/TS as clinically indicated.'
