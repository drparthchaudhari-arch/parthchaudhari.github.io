...
const WordVet = {
  ...
  generateWords(level) {
    ...
    // Add vet words
    while (
      this.words.filter((w) => this.vetWords.includes(w)).length < vetWordCount
    ) {
      const word =
        this.vetWords[Math.floor(Math.random() * this.vetWords.length)]
    ...
    while (this.words.length < wordCount) {
      const word =
        this.normalWords[Math.floor(Math.random() * this.normalWords.length)]
      if (!usedWords.has(word) && word.length >= 3 && word.length <= 10) {
        this.words.push(word)
        usedWords.add(word)
      }
  ...
  render(container) {
    ...
    cells.forEach((cell) => {
      ...
      cell.addEventListener('touchstart', (e) => this.handleStart(e), {
        passive: false,
      })
