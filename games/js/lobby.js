...
const LobbySystem = {
  ...
  updateLobbyUI() {
    ...
    // Update start button
    if (startBtn) {
      const allReady = this.currentLobby.players.every(
        (p) => p.ready || p.isHost
      )
  ...
  startPolling() {
    this.pollingInterval = setInterval(() => {
      ...
      if (this.currentLobby) {
        const updated = this.lobbies.find(
          (l) => l.code === this.currentLobby.code
        )
        if (updated) {
  ...
  updateLobbyList() {
    ...
    const availableLobbies = this.lobbies.filter(
      (l) => l.status === 'waiting' && l.players.length < 4
    )

    if (availableLobbies.length === 0) {
      list.innerHTML =
        '<p style="color: var(--text-secondary); text-align: center;">No active lobbies</p>'
      return
    }

    list.innerHTML = availableLobbies
      .map(
        (lobby) => `
