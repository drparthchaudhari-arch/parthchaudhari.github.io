...
function App() {
  ...
  const [cellStates, setCellStates] = useState<
    { isSelected: boolean; isFound: boolean }[]
  >([])
  ...
  const handleEnd = () => {
    ...
    if (currentWord.length >= 3) {
      ...
      if (
        currentLevel.words.includes(upperWord) &&
  ...
  const getCellStyle = (
    cell: Cell & { isSelected: boolean; isFound: boolean }
  ): string => {
    // Check if this cell is part of a hint
    const isHint =
      showHint &&
      hintPositions.some((p) => p.row === cell.row && p.col === cell.col)
    ...
    if (cell.isSelected)
      return 'bg-gradient-to-br from-blue-400 to-blue-600 text-white shadow-lg scale-110'
    if (isHint)
      return 'bg-gradient-to-br from-yellow-300 to-amber-400 text-amber-900 shadow-lg animate-pulse'
    ...
    <div className='min-h-screen bg-gradient-to-br from-indigo-900 via-purple-900 to-pink-800 flex flex-col overflow-hidden'>
      ...
      <div className='relative z-10 w-full max-w-lg mx-auto px-4 mt-4'>
        <div className='flex items-center justify-between text-white/80 text-sm mb-1'>
          <span>Progress</span>
          <span>
            {foundWords.length}/{currentLevel.words.length} words
          </span>
        </div>
          ...
          <div className='bg-white/10 backdrop-blur-lg rounded-2xl p-4 shadow-xl border border-white/20 min-h-[70px] flex items-center justify-center'>
            <p
              className={`text-2xl font-bold tracking-widest uppercase ${
                ...
                currentLevel.words.includes(currentWord.toUpperCase()) &&
                !foundWords.includes(currentWord.toUpperCase())
                  ? 'text-emerald-400'
                  : 'text-white'
              }`}
            >
              {currentWord || (
                <span className='text-white/40'>Drag to spell a word...</span>
              )}
            </p>
          </div>
        </motion.div>
            ...
            <div className='bg-white/10 rounded-xl p-4'>
              ...
              <div className='flex flex-wrap gap-1'>
                {foundWords.map((word) => (
                  <span
                    key={word}
                    className={`bg-gradient-to-r ${wordColors[word]} text-white text-xs px-2 py-1 rounded`}
                  >
                    {word}
                  </span>
                ))}
